...
insert into role_permission(role_id,resource_id,permission) values((select id from role where name='ROLE_ADMIN'),(select id from resource where name='roles'),0xff);
insert into resource(name) values ('users');
insert into role_permission(role_id,resource_id,permission) values((select id from role where name='ROLE_ADMIN'),(select id from resource where name='users'),0xff);
insert into resource(name) values ('resources');
insert into role_permission(role_id,resource_id,permission) values((select id from role where name='ROLE_ADMIN'),(select id from resource where name='resources'),0xff);
